...
public class DesenharSetaRespostaAutomato extends View
{
    public DesenharSetaRespostaAutomato(Context context) {
        super(context);
    }

    public DesenharSetaRespostaAutomato(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public DesenharSetaRespostaAutomato(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public DesenharSetaRespostaAutomato(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void desenharSetas(Canvas canvas){
        GUI.getGui().getItemTelaRespostaAutomato().limparTelaGradeSimbolos();
        GUI.getGui().getItemTelaRespostaAutomato().limparTelaGradeEstados();
        ArrayList<Estado> estados = GUI.getGui().getItemTelaRespostaAutomato().getItemTelaDesenhoAutomato().getListaEstadosAutomato();
        for(int i = 0; i < estados.size(); i++){
            ...
            for(int j = 0; j < transicoes.size(); j++) {
                ...
                if(transicao.getCordenadas_seta()[0] != transicao.getCordenadas_seta()[2] || transicao.getCordenadas_seta()[1] != transicao.getCordenadas_seta()[3]) {
                    ...
                    GUI.getGui().getItemTelaRespostaAutomato().desenharSetaMesmoAutomato(estados.get(i), transicao);
                }
            }
        }
    }
    ...
    private void montarSeta(Canvas canvas, float startX, float startY, float mX, float mY, Transicao transicao, Estado estado) {
        ...
        mY = mY + (GUI.getGui().getItemTelaRespostaAutomato().getItemTelaDesenhoAutomato().getHeiht()/2);
        ...
        GUI.getGui().getItemTelaRespostaAutomato().desenharSimbolosSeta(estado, transicao, cor);
